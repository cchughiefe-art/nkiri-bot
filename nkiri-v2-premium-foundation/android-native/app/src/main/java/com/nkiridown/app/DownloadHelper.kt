...
fun queryDownloadSnapshot(context: Context, downloadId: Long): DownloadSnapshot? {
    ...
    manager.query(DownloadManager.Query().setFilterById(downloadId))?.use { cursor ->
        ...
        fun long(name: String): Long = cursor.getColumnIndex(name).takeIf { it >= 0 }?.let(cursor::getLong) ?: 0L
        fun int(name: String): Int = cursor.getColumnIndex(name).takeIf { it >= 0 }?.let(cursor::getInt) ?: 0
